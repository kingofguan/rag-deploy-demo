...
output "apprunner_service_role_arn" {
  ...
  description = "复制到 GitHub Secret: ACCESS_ROLE_ARN"
}
output "apprunner_instance_role_arn" {
  value = aws_iam_role.apprunner_instance_role.arn
  description = "App Runner Instance Role ARN (用于运行时访问 Secrets)"
}
output "apprunner_service_arn" {
  value = can(aws_apprunner_service.rag_app_service[0].arn) ? aws_apprunner_service.rag_app_service[0].arn : null
  description = "复制到 GitHub Secret: APP_RUNNER_SERVICE_ARN (可选)"
}
output "apprunner_url" {
  value = can(aws_apprunner_service.rag_app_service[0].service_url) ? "https://${aws_apprunner_service.rag_app_service[0].service_url}" : null
  description = "RAG 应用的公开访问 URL"
}
output "apprunner_service_name" {
  value = "bee-edu-rag-service"
  description = "复制到 GitHub Secret: APPRUNNER_SERVICE_NAME"
}